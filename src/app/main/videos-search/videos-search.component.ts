...
export class VideosSearchComponent {
  ...
  doSearch(event): void {
    VideosSearchComponent.mainload=false;
    if (this.loadingInProgress)
    {
      return;
    }
      
    if(this.searchForm.value.query.trim().length === 0)
    {
      return;
    }
      
    if(this.last_search && this.last_search === this.searchForm.value.query) {
