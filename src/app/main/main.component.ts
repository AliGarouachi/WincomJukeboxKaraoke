...
export class MainComponent implements AfterViewInit {
  ...
  constructor(
    ...
    setInterval(() => {
        document.getElementById('search').hidden=VideosSearchComponent.mainload;
      }, 50);
  ...
  checkAddToPlaylist(video: any): void {
    if (!this.videoPlaylist.some((e) => e.id === video.id)) {
      ...
      setTimeout(() => {
        let topPos = document.getElementById(this.videoPlaylist[0].id).offsetTop;
  ...
  returnmain() : void {
    VideosSearchComponent.mainload=true;
  }
  searchMore(): void {
